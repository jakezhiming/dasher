...
from src.constants.screen import WIDTH
from src.constants.camera import (
    CAMERA_RIGHT_BOUNDARY_FACTOR,
...
def update_scroll(player, camera_x):
    ...
    dynamic_left_boundary = max(
        0, player.furthest_right_position - MAX_BACKTRACK_DISTANCE
    )
    ...
    # Follow player when moving left
        ...
        camera_x = max(
            dynamic_left_boundary, player.x - WIDTH * CAMERA_LEFT_BOUNDARY_FACTOR
        )
