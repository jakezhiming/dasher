...
from src.constants.player import PLAYER_WIDTH, PLAYER_HEIGHT
from src.constants.paths import (
    ...
    PLAYER_DUST_WALK_PATH,
    PLAYER_DUST_DOUBLE_JUMP_PATH,
    GROUND_TEXTURE_PATH,
    PLATFORM_TEXTURE_PATH,
    COIN_PATH,
    POWERUP_SPEED_PATH,
    POWERUP_FLYING_PATH,
    POWERUP_INVINCIBILITY_PATH,
    POWERUP_LIFE_PATH,
    SPIKES_PATH,
    FIRE_PATH,
    SAW_PATH,
...
from src.constants.game_objects import COIN_SIZE, POWERUP_SIZE
...
USE_CACHED_BACKGROUND = (
    os.getenv("USE_CACHED_BACKGROUND", "true" if IS_WEB else "false").lower() == "true"
)
        ...
        for path in bg_paths:
            try:
                ...
                scaled_layer = pygame.transform.scale(
                    layer, (new_width, PLAY_AREA_HEIGHT)
                )
...
def extract_dust_frames(action, num_frames):
    ...
    for i in range(num_frames):
        ...
        new_height = int(
            new_width * (sheet_height / frame_width)
...
def load_game_object_textures():
    ...
    try:
        ...
        if (
            coin_sprite.get_width() != COIN_SIZE
            or coin_sprite.get_height() != COIN_SIZE
        ):
    ...
    try:
        ...
        for p_type, path in powerup_paths.items():
            try:
                ...
                if (
                    powerup_sprites[p_type].get_width() != POWERUP_SIZE
                    or powerup_sprites[p_type].get_height() != POWERUP_SIZE
                ):
                    powerup_sprites[p_type] = pygame.transform.scale(
                        powerup_sprites[p_type], (POWERUP_SIZE, POWERUP_SIZE)
                    )
            except Exception as e:
...
def create_cached_background(screen_width):
    ...
    try:
        ...
        if CACHED_BACKGROUND is None or screen_width != SCREEN_WIDTH:
            ...
            CACHED_BACKGROUND = pygame.Surface(
                (screen_width, PLAY_AREA_HEIGHT), pygame.SRCALPHA
            )
