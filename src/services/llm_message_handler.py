...
import asyncio
from src.constants.messages import PERSONALITIES, DEFAULT_PERSONALITY
...
class LLMMessageHandler:
    def __init__(self):
        ...
        if not IS_WEB:
            if not OPENAI_API_KEY:
                logger.warning(
                    "OPENAI_API_KEY not found in environment variables. Use default messages."
                )
                self.client = None
            else:
                try:
                    ...
                    logger.error(
                        f"Failed to initialize OpenAI client: {e}. Use default messages."
                    )
                    self.client = None
        elif IS_WEB:
    ...
    def add_to_conversation_history(self, original_message, rephrased_message):
        ...
        self.conversation_history.append(
            {"original": original_message, "rephrased": rephrased_message}
        )
...
Do not use any emojis, line breaks, or special characters.
...
{history_context}Message to rephrase: \"{original_message}\""""
            ...
            if IS_WEB:
                ...
                try:
                    complete_message = await asyncio.wait_for(
                        self._fetch_openai_web(prompt, original_message), timeout
                    )
                except asyncio.TimeoutError:
                    logger.error(
                        f"Proxy Server API call timed out after {timeout} seconds"
                    )
                    return original_message
            else:
                ...
                try:
                    ...
                    logger.error(
                        f"API call timed out after {timeout} seconds. Switch to default messages."
                    )
                    self.client = None
                    return original_message
    ...
    async def _fetch_openai_web(self, prompt, original_message, timeout=5):
        ...
        try:
            ...
            if not callable(window.fetchLLMResponse):
                logger.error(
                    f"fetchLLMResponse exists but is not callable: {window.fetchLLMResponse}"
                )
                return original_message
            ...
            if llm_response:
                if "DasherError" in llm_response:
                    logger.error(
                        f"Proxy Server returned an error: {llm_response}. Switch to default messages."
                    )
    ...
    def change_personality(self):
        ...
        new_personality = random.choice(
            [p for p in PERSONALITIES if p != self.personality]
        )
