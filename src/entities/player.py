...
from src.constants.player import (
    ...
    PLAYER_INITIAL_Y,
    GRAVITY,
    FLYING_GRAVITY_REDUCTION,
    INVINCIBILITY_DURATION,
    INVINCIBILITY_FROM_DAMAGE_DURATION,
    IMMOBILIZED_DURATION,
    INITIAL_LIVES,
...
from src.constants.colors import DARK_RED, LIGHT_BLUE
from src.constants.animation import (
    PLAYER_ANIMATION_SPEED,
    DEATH_ANIMATION_FRAME_DELAY,
    SPEED_BOOST_ANIMATION_FACTOR,
)
from src.utils.utils import collide
from src.core.assets_loader import get_frame, player_frames, get_cloud_image
...
class Player:
    def __init__(self, x=PLAYER_INITIAL_X, y=PLAYER_INITIAL_Y):
        ...
        self.trail_positions = (
            []
        )  # Store previous positions for speed boost trail effect
        self.flying = False
        self.flying_timer = 0
    ...
    def draw(self, screen, camera_x):
        ...
        if self.dying:
            if current_time - self.death_animation_timer > DEATH_ANIMATION_FRAME_DELAY:
                ...
                if self.death_animation_frame >= len(player_frames["death_right"]):
                    ...
                    self.death_animation_frame = (
                        len(player_frames["death_right"]) - 1
                    )  # Stay on last frame
        else:
        ...
        if self.flying and not self.dying:
            ...
            if not hasattr(self, "cloud_image"):
                ...
                if original_cloud is not None:
                    ...
                    self.cloud_image = pygame.transform.scale(
                        original_cloud, (cloud_width, cloud_height)
                    )
                else:
                    ...
                    try:
                        ...
                        cloud_height = self.height * 3
                        self.cloud_image = pygame.transform.scale(
                            self.cloud_image, (cloud_width, cloud_height)
                        )
            ...
            if self.direction == "right":
                ...
                cloud_image = pygame.transform.flip(
                    self.cloud_image, True, False
                )  # Flip horizontally
            ...
            bob_offset = (
                math.sin(pygame.time.get_ticks() * 0.005) * 2
    ...
    def _draw_dust_effects(self, screen, screen_x):
        ...
        if self.show_double_jump_dust:
            ...
            if current_time - self.double_jump_dust_timer > double_jump_dust_speed:
                ...
                if self.double_jump_dust_frame >= len(
                    player_frames["dust_double_jump"]
                ):
                    self.show_double_jump_dust = False
            ...
            if self.show_double_jump_dust:
                ...
                if self.speed_boost:
                    ...
                    larger_frame = pygame.transform.scale(
                        dust_frame,
                        (
                            int(dust_frame.get_width() * 1.2),
                            int(dust_frame.get_height() * 1.2),
                        ),
                    )
                    larger_x = (
                        dust_x
                        - (larger_frame.get_width() - dust_frame.get_width()) // 2
                    )
                    larger_y = (
                        dust_y
                        - (larger_frame.get_height() - dust_frame.get_height()) // 2
                    )
        ...
        if abs(self.vx) >= 5:
            ...
            for i in range(1):
                ...
                line_end_x = line_start_x + (
                    -line_length if self.direction == "right" else line_length
                )
                pygame.draw.line(
                    screen,
                    LIGHT_BLUE,
    ...
    def update(self, floors, platforms, obstacles, coins, power_ups):
                ...
                if player_rect.colliderect(obstacle_rect):
                    ...
                    # Skip collision handling if player is invincible from power-up (not from damage)
                        ...
                        if self.vx > 0:  # Moving right
                            ...
                            )  # Place player to the left of the obstacle
                        elif self.vx < 0:  # Moving left
                            self.x = (
                                obstacle_rect.x + obstacle_rect.width
                            )  # Place player to the right of the obstacle
        ...
        dynamic_left_boundary = max(
            0, self.furthest_right_position - MAX_BACKTRACK_DISTANCE
        )
        ...
        for platform in platforms:
            ...
            if collide(self, platform):
                ...
                if self.vy > 0 and (  # Moving downward
                    ...
                    or self.prev_y + self.height  # Either feet are near the top
                    <= platform.y
                ):  # OR was above the platform in previous frame
            ...
            # This handles cases where player moved so fast they went from below to above the platform in one frame
                ...
                not collide(self, platform)
                and self.prev_y + self.height  # Not currently colliding
                > platform.y + platform.height
                and self.y < platform.y  # Was below the platform
                and self.x + self.width > platform.x  # Now above the platform
        ...
        if self.y > PLAY_AREA_HEIGHT:
            ...
            if self.lives > 0:
                ...
                from src.ui.ui import message_manager, set_hearts_flash
                ...
                elif self.lives == 1 and not message_manager.has_shown_message(
                    "last_life"
                ):
                ...
                if self.lives > 0:
                    ...
                    if collided_obstacle:
                        if collided_obstacle.type == "spikes":
                            ...
                            collided_obstacle.type == "bomb"
                            and collided_obstacle.exploded
                        ):
                            message_manager.set_message(random.choice(BOMB_MESSAGES))
                        else:
                            message_manager.set_message(
                                random.choice(OBSTACLE_MESSAGES)
                            )
                    ...
                    if collided_obstacle:
                        ...
                        if (
                            self.x + self.width / 2
                            < collided_obstacle.x + collided_obstacle.width / 2
                        ):
        ...
        for coin in coins[:]:
            if collide(self, coin):
                ...
                from src.ui.ui import set_score_highlight
        ...
        for power_up in power_ups[:]:
            if collide(self, power_up):
                ...
                effect_manager.create_powerup_effect(
                    power_up.x, power_up.y, power_up.type
                )

                # Import message_manager here to avoid circular imports
                from src.ui.ui import message_manager
        ...
        if (
            self.speed_boost
            and current_time - self.speed_boost_timer > SPEED_BOOST_DURATION
        ):
            self.speed_boost = False
        ...
        if self.invincible:
            if (
                self.invincible_from_damage
                and current_time - self.invincible_timer
                > INVINCIBILITY_FROM_DAMAGE_DURATION
            ):
                self.invincible = False
            elif (
                not self.invincible_from_damage
                and current_time - self.invincible_timer > INVINCIBILITY_DURATION
            ):
                self.invincible = False
        ...
        if self.bonus_score_active:
            ...
            if score_increase >= self.bonus_score_increase_requirement:
                ...
                self.bonus_score_increase_requirement = min(
                    self.bonus_score_increase_max_requirement,
                    self.bonus_score_increase_requirement
                    + (self.bonus_score_period_count * 50),
                )

                # Reset the target reached celebration effect
                from src.ui.ui import reset_target_reached_celebration
        ...
        if not self.immobilized and not self.dying:
            ...
            if self.speed_boost:
                ...
                if random.random() < 0.2:  # 20% chance each frame
                    ...
                    particle_x = self.x + (
                        0 if self.direction == "right" else self.width
                    )
