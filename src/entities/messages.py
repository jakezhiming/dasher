...
from src.services.llm_message_handler import LLMMessageHandler
from src.utils.logger import get_module_logger
from src.utils.compat import IS_WEB
from src.constants.messages import DEFAULT_MESSAGES
...
class StatusMessageManager:
    def __init__(self):
        ...
        self.default_message_delay = (
            DEFAULT_MESSAGE_DELAY  # 5 seconds between default messages
        )
    ...
    def set_message(self, message):
        ...
        if (
            message
            and message not in self.message_queue
            and message != self.target_message
            and message != self.previous_message
            and message != self.current_full_message
        ):
            ...
            if not self.llm_handler.is_available():
                ...
                if IS_WEB:
                    ...
                    asyncio.run_coroutine_threadsafe(
                        self._process_with_llm(message), self.loop
                    )
    ...
    async def _process_with_llm(self, original_message):
        ...
        try:
            ...
            full_response = await self.llm_handler.get_streaming_response(
                original_message
            )
            ...
            if (
                full_response
                and full_response not in self.message_queue
                and full_response != self.target_message
                and full_response != self.previous_message
                and full_response != self.current_full_message
            ):
            ...
            if (
                original_message not in self.message_queue
                and original_message != self.target_message
                and original_message != self.previous_message
                and original_message != self.current_full_message
            ):
    ...
    def update(self):
        ...
        if self.display_index < len(self.target_message):
            ...
            if (
                self.message_queue
                and current_time - self.message_completed_time
                > self.message_transition_delay
            ):
                self._load_next_message()
    ...
    def can_show_default_message(self):
        ...
        return (
            current_time - self.last_message_time > self.default_message_delay
            and not self.message_queue
            and (
                not self.target_message
                or (
                    self.target_message
                    and self.display_index == len(self.target_message)
                )
            )
        )
...
def get_status_message():
    ...
    if message_manager.can_show_default_message():
        ...
        current_default_message = DEFAULT_MESSAGES[
            message_manager.default_message_index
        ]

        # Only set the message if it's different from the current and previous messages
        if (
            current_default_message != message_manager.current_full_message
            and current_default_message != message_manager.previous_message
        ):
            ...
            message_manager.default_message_index = (
                message_manager.default_message_index + 1
            ) % 5
