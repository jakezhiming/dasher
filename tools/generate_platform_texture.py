...
for x in range(0, TEXTURE_WIDTH, plate_width):
    ...
    for y in range(TEXTURE_HEIGHT):
        ...
        color_value = int(
            MEDIUM_GRAY[0] + (LIGHT_GRAY[0] - MEDIUM_GRAY[0]) * (1 - gradient_factor)
        )
        platform_texture.set_at(
            (x + plate_width // 2, y), (color_value, color_value, color_value + 10)
        )
    ...
    pygame.draw.line(
        ...
        DARK_GRAY,
        (x, TEXTURE_HEIGHT // 2),
        (x + plate_width, TEXTURE_HEIGHT // 2),
        1,
    )
...
for x in range(0, TEXTURE_WIDTH, plate_width):
    ...
    pygame.draw.circle(
        platform_texture, RIVET_COLOR, (x + plate_width - 3, TEXTURE_HEIGHT - 3), 2
    )

    # Middle rivets
    pygame.draw.circle(platform_texture, RIVET_COLOR, (x + 2, TEXTURE_HEIGHT // 2), 2)
    pygame.draw.circle(
        platform_texture, RIVET_COLOR, (x + plate_width - 3, TEXTURE_HEIGHT // 2), 2
    )
