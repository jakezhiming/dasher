...
def inject_ui_panels(index_html_path):
    ...
    try:
        ...
        css_file_path = "web/panels.css"
        html_file_path = "web/panels.html"
        ...
        if "</head>" in html_content:
            html_content = html_content.replace(
                "</head>", '<link rel="stylesheet" href="panels.css"></head>'
            )
...
def build_with_pygbag():
    ...
    folders_not_required = ["tools", "logs", "web"]
    temp_files = {}
    temp_folders = {}
    ...
    try:
        ...
        print(
            "Moving files and folders not required for the build out of the build directory..."
        )
    ...
    if os.path.exists("web/web_api.js"):
        print("Copying web_api.js to build/web directory...")
        shutil.copy("web/web_api.js", "build/web/web_api.js")
        ...
        if os.path.exists(index_html_path):
            ...
            if "</head>" in html_content:
                html_content = html_content.replace(
                    "</head>", '<script src="web_api.js"></script></head>'
                )
